import { GoogleAuthProvider, signInWithPopup } from "firebase/auth";
import { auth } from "../services/firebase";
import { useAuthState } from "react-firebase-hooks/auth";
import { useEffect } from "react";
import { useNavigate } from "react-router-dom";


export const Login = () => {

    const date: Date = new Date();
    const year: number = date.getFullYear();
    const [user, loading]: any = useAuthState(auth);
    const navigate: any = useNavigate();
    
    console.log("Current user:", user);
    console.log("Loading:", loading);

    useEffect(() => {
        // Si el usuario está autenticado, navegar al chat
        if (user && !loading) {
            console.log("User authenticated, navigating to chat");
            navigate("/chat");
        }
    }, [user, loading, navigate]);

    const googleLogin = async () => {
        try {
            const provider = new GoogleAuthProvider();
            const result = await signInWithPopup(auth, provider);
            console.log("Login successful:", result.user);
        } catch (error) {
            console.error("Error during login:", error);
        }
    };


  return (
    <div className="bg-mint-light h-screen w-full flex items-center justify-center">
      <div className="w-full max-w-110 px-6">
      <div className="bg-white rounded-2xl shadow-soft p-10 sm:p-12 flex flex-col items-center text-center">
        <div className="mb-8 relative w-20 h-20 flex items-center justify-center">
          <svg
            fill="none"
            height="80"
            viewBox="0 0 80 80"
            width="80"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M15 25C15 16.7157 21.7157 10 30 10H50C58.2843 10 65 16.7157 65 25V40C65 48.2843 58.2843 55 50 55H30C28.5 55 27 54.8 25.5 54.5L15 60V25Z"
              fill="#9FBDAF"
              fill-opacity="0.8"
            ></path>
            <path
              d="M30 35C30 26.7157 36.7157 20 45 20H65C73.2843 20 80 26.7157 80 35V50C80 58.2843 73.2843 65 65 65H45C43.5 65 42 64.8 40.5 64.5L30 70V35Z"
              fill="#5E8175"
            ></path>
            <circle
              cx="48"
              cy="42.5"
              fill="white"
              fill-opacity="0.9"
              r="2.5"
            ></circle>
            <circle
              cx="56"
              cy="42.5"
              fill="white"
              fill-opacity="0.9"
              r="2.5"
            ></circle>
            <circle
              cx="64"
              cy="42.5"
              fill="white"
              fill-opacity="0.9"
              r="2.5"
            ></circle>
          </svg>
        </div>

        <h1 className="text-2xl font-bold text-slate-800 mb-2 tracking-tight">
          Bienvenido a SageChat
        </h1>
        <p className="text-slate-500 mb-10 font-medium">
          Por favor, inicia sesión para continuar
        </p>
        <button className="w-full bg-white text-slate-700 font-medium py-3.5 px-4 rounded-xl shadow-sm hover:shadow-md hover:bg-slate-50 transition-all duration-200 flex items-center justify-center gap-3 border border-mint-border group btn-google" 
            
            onClick={googleLogin}
        >
          <svg
            className="w-5 h-5 group-hover:scale-110 transition-transform duration-200"
            viewBox="0 0 24 24"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
              fill="#4285F4"
            ></path>
            <path
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
              fill="#34A853"
            ></path>
            <path
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
              fill="#FBBC05"
            ></path>
            <path
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
              fill="#EA4335"
            ></path>
          </svg>
          Iniciar sesión con Google
        </button>
        <div className="mt-8 flex items-center justify-center gap-4 text-xs text-slate-400">
          <a className="hover:text-primary transition-colors" href="#">
            Terms of Service
          </a>
          <span className="w-1 h-1 rounded-full bg-slate-300"></span>
          <a className="hover:text-primary transition-colors" href="#">
            Privacy Policy
          </a>
        </div>
      </div>
      <p className="text-center text-xs text-slate-400 mt-6">
        © {year} SageChat Inc. All rights reserved.
      </p>
    </div>
    </div>
  );
};

export default Login;